...
const RequestForm = () => {
  ...
  const handleSubmit = async () => {
    const rollno = localStorage.getItem("studentRollno"); // ✅ get rollno from localStorage
    if (!rollno || !reason || !incharge || !hod) {
    ...
    try {
      ...
      formData.append("to", role);
      
      if (file) formData.append("proofs", file);

      const res = await axios.post(`${API_URL}/request/send`, { rollno, reason });
